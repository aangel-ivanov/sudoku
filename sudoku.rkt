...
(define (get-nonet s row col)
  (local [; (define nonet (+ (* 3 (floor (/ (- row 1) 3))) (floor (/ (- column 1) 3)) 1))
...
;(display (cond [(and (= (modulo row 3) 1) (= col 1)) "+---+---+---+\n" ""]))
;(display (cond [(= (modulo col 3) 1) "|" ""]))
;(display (cond [(zero? (get-val s row col)) "?" (get-val s row col)]))
;(display (cond [(= col 9) "|\n" ""]))


(define (set s row col val)
...
(define (value-okay? s row col)
  (and (not (zero? (get-val s row col)))
       ...
       (nonet-okay? (get-nonet s row col) row col (get-val s row col))))
...
(define (nonet-okay? vals row col val)
...
(define (step-solve-sudoku puzzle)
  (local [(define (iter s row col back)
            ...
            (cond [(= row 10) (display-sudoku s)] ; Puzzle is solved
                  [(get-val (second puzzle) row col) ; Ran into fixed value that must be skipped
                   (cond [back ; we are back tracking and need to continue to back track
                          (cond [(= col 1) (iter s (sub1 row) 9 #t)]
                                [else (iter s row (sub1 col) #t)])]
                         [else (cond [(= col 9) (iter s (add1 row) 1 #f)]
                                     [else (iter s row (add1 col) #f)])])]
                  [(and (value-okay? s row col) (not back)) ; we found a number that works so skip it
                   (cond [(= col 9) (iter s (add1 row) 1 #f)]
                         [else (iter s row (add1 col) #f)])]
                  [(< (get-val s row col) 9) ; try a different value in that space
                   (iter (set s row col (add1 (get-val s row col))) row col #f)]
                  [else ; backtract
                   (cond [(= col 1) (iter (set s row col 0) (sub1 row) 9 #t)]
                         [else (iter (set s row col 0) row (sub1 col) #t)])]))]

    (iter (first puzzle) 1 1 #f)))
    

;; This attaches the list of boolean values to the sudoku.
;; The bools are used to see if a value in a sudoku was given.
(define (create-sudoku-obj s)
  (local [(define (iter s-bools row col)
            (cond [(> col 9) (iter s-bools (add1 row) 1)]
                  [(> row 9) (list s s-bools)]
                  [(zero? (get-val s row col))
                   (iter (set s-bools row col #f) row (add1 col))]
                  [else (iter (set s-bools row col #t) row (add1 col))]))]
